...
# # betternet.fc = nn.Linear(in_features=2048, out_features=259, bias=True)
...
# loss_func = nn.CTCLoss(blank=10)
# loss_func = nn.CrossEntropyLoss()
# loss_func = nn.MultiLabelMarginLoss()
# loss_func = nn.NLLLoss()
# params = as3_cnn.parameters()
params = betternet.parameters()
...
for epoch in range(max_epoch):
    ...
    for step, i in enumerate(train_dl):
        ...
        # pred_cnn, feature = betternet(img)
        ##############################################################################
        caption_train = label_oh.reshape(batch_size, ALL_CHAR_SET_LEN, MAX_CAPTCHA)  # (128, 259) -> (128, 37, 7)
        # caption_train = torch.argmax(caption_train, 1)
        caption_train = caption_train.permute(0, 2, 1)  # (128, 37, 7) -> (128, 7, 37)

        # teach_ratio = epoch/max_epoch
        # teach_ratio = random.random()
        # feature = torch.flatten(pred_cnn, 1)
        # pred_lstm = lstm(feature, caption_train)  # (128, 512, 5, 2), (128, 7, 37), float
        # pred_lstm = pred_lstm.permute(0, 2, 1)  # (128, 37, 7)
        # pred_lstm = torch.flatten(pred_lstm, 1)  # 128, 259

        # loss = loss_func(pred_lstm, label_oh)  # softmax(128, 259) vs. one-hot(128, 259)
        loss = loss_func(pred_cnn, label_oh)
        if loss < threshold:
            isConverged = True

        optimizer.zero_grad()
        loss.backward()
        # nn.utils.clip_grad_norm_(list(betternet.parameters()) + list(lstm.parameters()), 5)
        optimizer.step()
        ##############################################################################
...
# hidden_size = 8
# vocab_size = 37  # ALL_CHAR_SET_LEN
# betternet_model = resnet.resnet18(pretrained=False)
# lstm_model = LSTM(cnn_dim=cnn_dim, hidden_size=hidden_size, vocab_size=vocab_size)
# if torch.cuda.is_available():
#     betternet_model = betternet_model.cuda()
#     lstm_model = lstm_model.cuda()
# betternet_model.load_state_dict(torch.load(cnn_model_path))
# betternet_model = betternet_model.to(device)
