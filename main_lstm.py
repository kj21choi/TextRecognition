...
class Mydataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return img, np.array(label_oh), label, target_len
...
class LSTM(nn.Module):
    ...
    def forward(self, features, captions, isTraining=True):
        ...
        outputs = torch.zeros((batch_size, captions.size(1), self.vocab_size)).to(device)  # 128 * 7 * 37
        ...
        for t in range(captions.size(1)):  # 0 ~ 6
            ...
            out = self.fc_out(hidden_state)  # 256 -> 37 predict next state
            outputs[:, t, :] = out
            if not isTraining:
                outputs = self.softmax(outputs)  # 0 ~ 1 activate
        ##############################################################################
        return outputs
...
for epoch in range(max_epoch):
    if isConverged:
        break
    for step, i in enumerate(train_dl):
        img, label_oh, label, target_lengths = i
        ...
        pred_lstm = lstm(feature, label_oh_reshape, True)  # (128, 512, 5, 2), (128, 7, 37), float
        # outputs = lstm(feature, caption_train, True)  # (128, 512, 5, 2), (128, 7), float
        outputs = torch.flatten(pred_lstm, 1)  # 128, 259
        loss = loss_func(outputs, label_oh)  # softmax(128, 259) vs. one-hot(128, 259)
        # for end-to-end learning
        # loss = loss_func(outputs, caption_oh, input_lengths, target_lengths)  # softmax(128, 259) vs. one-hot(128, 259)
...
lstm_model_path = './trained_lstm_model_end_to_end.pth'
...
with torch.no_grad():
    for step, (img, label_oh, label, length) in enumerate(test_dl):
        char_count = 0
        img = Variable(img).to(device)
        label_oh = Variable(label_oh.float()).to(device)
        pred_cnn, feature = betternet(img)
